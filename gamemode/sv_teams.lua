...
concommand.Add("car_jointeam", function (ply, com, args)
	...
	if newteam == 1 and curteam ~= 1 then
	...
	elseif newteam >= 2 and newteam <= 3 and newteam ~= curteam then
...
function GM:CheckTeamBalance()
	if !self.TeamBalanceCheck or self.TeamBalanceCheck < CurTime() then
		self.TeamBalanceCheck = CurTime() + 3 * 60 -- check every 3 minutes

		local diff = team.NumPlayers(2) - team.NumPlayers(3)
		if diff < -1 or diff > 1 then -- teams must be off by more than 2 for team balance
	...
	if self.TeamBalanceTimer and self.TeamBalanceTimer < CurTime() then
